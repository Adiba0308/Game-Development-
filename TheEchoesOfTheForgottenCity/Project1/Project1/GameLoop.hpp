...
void TriggerSkipLocation() { // Developer/debug function that instantly skips the current location or level, advancing the player to the next area
...
void UpdateSkipMechanism(float dt) { // Detects double-press of 'S' within 3 seconds as the skip trigger; resets if timeout expires
...
void UpdatePlay(float dt) { // Core Level 1 gameplay update — handles player movement, jumping, gravity, attacks, health, fire/blade hazards, bubble interactions, gate logic, and location transitions
	if (isTransitioning) {
...
void fixedUpdate() { // Periodic fixed-timestep update loop called by iGraphics — polls keyboard for menu/escape/skip/level-complete inputs and dispatches to per-level and per-puzzle update functions
	...
	if (enterDown && !enterPrev) {
		if (gameState == STATE_LEVEL_COMPLETED) {
			if (currentLevel == 1) {
				...
				// Force reload of leaderboard contents
				leaderboard.loaded = false;
				gameState = STATE_SCOREBOARD;
				printf("Transitioning to Leaderboard...\n");
			}
		}
	}
	enterPrev = enterDown;
...
// Update animations
void AnimationUpdate() { // Timer-driven animation tick (~60 FPS) — updates button float offsets, title glow, slideshow progression, game-over countdown, and death/respawn logic
	globalTime += 0.1f;
