...
void LoadSlideshow() { // Initializes the intro story slideshow — loads 6 narrative slides with images, titles, and text describing the game's lore
...
void UpdateParticles() { // Updates floating dust particles in the slideshow — handles respawning, movement, and alpha fade-out
	for (int i = 0; i<50; i++) {
...
// Draw Slideshow
void DrawSlideshow() { // Renders the current slideshow slide with zoom effect, glowing text boxes, narrative text wrapping, and fade overlays
...
void UpdateSlideshow() { // Advances the slideshow state machine through fade-in, display, and fade-out phases; transitions to name input after the final slide
...
void SkipSlide() { // Allows the player to skip the current slide by immediately starting the fade-out transition
