...
void LoadLeaderboard() { // Reads all leaderboard entries from the text file (name|score format) into the in-memory leaderboard array
	leaderboard.count = 0;
...
void SaveLeaderboardEntry(const char* name, long score) { // Appends a single player name and score entry to the leaderboard text file
...
void ClearLeaderboardFile() { // Erases all leaderboard data by truncating the file and resetting the in-memory count to zero
	FILE* fp;
...
void SortLeaderboard() { // Sorts leaderboard entries in descending order by score using insertion sort
...
void InitLeaderboardButtons() { // Creates and positions the Back, Clear Leaderboard, and confirm-dialog (Yes/No) buttons for the leaderboard screen
...
void DrawOrnamentalLine(int x1, int y, int x2) { // Draws a decorative horizontal line with fading transparency from center outward, featuring a diamond ornament at center
	int cx = (x1 + x2) / 2;
	int hw = (x2 - x1) / 2;
	for (int i = 0; i < hw; i++) {
...
void DrawMedal(int cx, int cy, int rank) { // Draws a circular medal icon with rank-dependent colors (gold/silver/bronze) for top 3 leaderboard positions
	int r1, g1, b1; // outer
	int r2, g2, b2; // inner
	int r3, g3, b3; // text
	float outerR = 14.0f;
...
void DrawLeaderboardButton(Button& btn) { // Renders a styled leaderboard screen button with multi-layer glow on hover and glowing text
	int bx = btn.x;
...
void DrawLeaderboard() { // Renders the full leaderboard screen with ranked entries, medals, alternating rows, clear/back buttons, and an optional confirm-clear dialog overlay
	glEnable(GL_BLEND);
...
void DrawNameInput() { // Renders the player name input screen with a styled panel, blinking cursor, character progress bar, and keyboard instructions
	glEnable(GL_BLEND);
	glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void HandleNameInputKey(unsigned char key) { // Processes keyboard input on the name entry screen — Enter confirms and starts a new game, Escape cancels, Backspace deletes, printable chars are appended
	if (key == 13) { // Enter
		if (nameInputLen > 0) {
...
void HandleLeaderboardMouseMove(int mx, int my) { // Updates hover state for leaderboard and confirm-dialog buttons based on mouse position
	if (gameState == STATE_LEADERBOARD_CONFIRM) {
		for (int i = 0; i < 2; i++) {
...
void HandleLeaderboardClick(int mx, int my) { // Handles click actions on the leaderboard screen — Back returns to menu, Clear opens confirm dialog, Yes clears data, No cancels
	if (gameState == STATE_LEADERBOARD_CONFIRM) {
